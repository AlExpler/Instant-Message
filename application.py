...
class MyTableWidget(QWidget):
    def __init__(self, parent):
        ...
        self.nameBox = QGroupBox("Nickname")
        ...
        self.messageRecords = QLabel("<font color=\"#000000\">Welcome to the chat!</font>", self)
    ...
    def enter_line(self):
        if self.sendTo != self.sendComboBox.currentText():
            self.message_display_append("User is offline, we couldn't deliver the message")
            self.lineEdit.clear()
            return
    ...
    def message_display_append(self, newMessage, textColor = "#000000"):
        ...
        time.sleep(0.2) #это помогает установить панель внизу, после того как ALL сообщения уже добавлены
        self.scrollRecords.verticalScrollBar().setValue(self.scrollRecords.verticalScrollBar().maximum())
            ...
            if data != "":
                if "{CLIENTS}" in data:
                    ...
                    if not welcome[0][5:] == "":
                        ...
                        self.scrollRecords.verticalScrollBar().setValue(self.scrollRecords.verticalScrollBar().maximum())
                elif data[:5] == "{MSG}": #{MSG} включает в себя широковещательные и серверные сообщения
                    self.message_display_append(data[5:], "#006600")
                    self.scrollRecords.verticalScrollBar().setValue(self.scrollRecords.verticalScrollBar().maximum())
                else: #личное сообщение имеет формат NONE
                    self.message_display_append("{private}"+data, "#cc33cc")
                    self.scrollRecords.verticalScrollBar().setValue(self.scrollRecords.verticalScrollBar().maximum())
            time.sleep(0.1) #это для экономии времени цикла потока
    ...
    def connect_server(self):
        ...
        if port == "" or not port.isnumeric():
            self.portLineEdit.setText("33002")
            self.connStatus.setText("Status: "+"Incorrect port format")
            return 
        else:
            port = int(port)
        try:
            ...
            self.connStatus.setText("Status: "+"Connection declined")
            self.conn = socket.socket()
            return
        send_msg = bytes("{REGISTER}" + name, "utf-8")
        self.conn.send(send_msg)
        self.connected = True 
        self.connStatus.setText("Status: "+"Online")
        self.nameLineEdit.setReadOnly(True) #Эта настройка не работает должным образом
    ...
    def disconnect_server(self):
        ...
        self.conn.send(send_msg)
        self.connStatus.setText("Status: " + "Offline")
    ...
    def update_send_to_list(self, strList):
        ...
        print("Last choice:",index)
        if index != -1:
            self.sendComboBox.setCurrentIndex(index) #обновление, обслуживание приемника
        else:
            self.sendComboBox.setCurrentIndex(0) #обновление, приемник слева, по умолчанию "ALL"
 
    def send_choice(self,text):
        self.sendTo = text
        print(self.sendTo)
        self.sendChoice.setText("Send: "+text)
 
class Window(QMainWindow):
    def __init__(self):
        super(Window, self).__init__()
        self.setGeometry(50, 50, 500, 300)
        self.setWindowTitle("Instant Message")        
    ...
    def closeEvent(self, event):
        ...
        if close == QMessageBox.Yes:
            self.table_widget.disconnect_server() #отключиться от сервера перед выходом
            event.accept()
        else:
            event.ignore()
